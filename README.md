...
4. **Accept and provision.** Click “Accept estimate”, then use the “Create account” button to hit `/wp-json/ca/v1/portal/accept` and `/wp-json/ca/v1/portal/create-account`. A WordPress customer user should be created or updated.
...
3. Browse to `https://your-site.com/portal?estimateId=...&locationId=...&inviteToken=...`. The shortcode is no longer required—the plugin hijacks the route and bootstraps the React portal automatically.
