...
4. **Simulate acceptance.** Mark the estimate as accepted in GHL (or mock it via the API). The WordPress plugin will detect the accepted state, automatically create/update the customer user, and log the invite link to the browser console during development.
5. **Verify status.** Inspect the WordPress Users table, review the “Customer Account” card in the portal, and optionally call `/wp-json/ca/v1/portal/test-account?estimateId=ESTIMATE_ID&locationId=LOCATION_ID` to confirm invite metadata.
...
3. Browse to `https://your-site.com/portal?estimateId=...&locationId=...&inviteToken=...`. The shortcode is no longer required—the plugin hijacks the route and bootstraps the React portal automatically. Unauthenticated visitors are redirected to the WordPress login page, and `customer` users land on `/portal` immediately after signing in.
...
_Remember: Good code is written for humans to read, not just for computers to execute._
